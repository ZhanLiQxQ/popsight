...
AmazonCategoryPreset = Literal[
    ...
    "personal_care_otc",
    "cultural_specialty",
    # legacy aliases (still resolve to browse nodes)
    "grocery",
    "snacks",
    "supplements",
]
...
class AmazonPipelineRequest(BaseModel):
    ...
    def normalize_category_node(cls, value: object) -> str | None:
        ...
        if not s:
            return None
        # Swagger UI often leaves the placeholder literal "string"; treat as unset.
...
# Named bodies for Swagger UI (POST /api/pipeline/macro-cold-start).
...
class MacroColdStartResponse(BaseModel):
    ...
    rankedProductList: list[dict[str, Any]] = Field(
        default_factory=list,
        description="Phase 3 placeholder (empty until Trend Analyst ships).",
    )
    finalActionableList: list[dict[str, Any]] = Field(
        default_factory=list,
        description="Phase 3 placeholder (empty until Supply Planner ships).",
    )
    executiveSummary: str = Field(
        default="",
        description="Phase 3 placeholder (empty until Summary Agent ships).",
    )
    discoveryAborted: bool = Field(
        default=False,
        description="True when Node 0 produced no usable rising queries for any lane; Nodes 1–3 were skipped.",
    )
