"""
Product Discovery LangGraph — **Node 0–6**.

Per lane: Google Trends (Node 0) → MacroScout (Node 1) → Amazon Grocery Top 5 (Node 2) → FDA + GLiNER (Node 3).

After all lanes: **Node 4** rank → **Node 5** supply → **Node 6** Markdown summary (``discovery_post_pipeline`` demo logic).
"""
...
from .discovery_post_pipeline import (
    derive_scan_topic,
    lanes_compressed_to_external_by_category,
...
class DiscoveryPipelineRunner:
    ...
    async def _trend_analyst_agent(self, state: DiscoveryState) -> DiscoveryState:
        t0 = time.perf_counter()
        try:
            user_id = state.get("user_id") or "default-user"
            lanes = list(state.get("lanes") or [])
            ext = lanes_compressed_to_external_by_category(lanes)
            topic = derive_scan_topic(lanes)

            def _run() -> tuple[dict[str, Any], list[dict[str, Any]]]:
                bundle, internal = load_pipeline_bundle(topic, external_by_category=ext)
                ranked = run_product_ranking(
                    topic=topic,
                    external_by_category=ext,
                    internal_metrics=internal,
                    catalog_excerpt="",
                    catalog_rag_chunks=list(bundle.get("catalog_rag_chunks") or []),
                )
                return bundle, ranked

            bundle, ranked = await asyncio.to_thread(_run)
            self.repository.add_agent_log(
                user_id=user_id,
                agent_name="Trend_Analyst_Agent",
                message=f"Node 4: ranked {len(ranked)} SKU(s).",
                log_type="info",
            )
            return {"pipeline_bundle": bundle, "ranked_product_list": ranked}
        finally:
            _discovery_timing_print("Node 4 (Trend Analyst / rank)", t0)

    async def _supply_planner_agent(self, state: DiscoveryState) -> DiscoveryState:
        t0 = time.perf_counter()
        try:
            user_id = state.get("user_id") or "default-user"
            bundle = dict(state.get("pipeline_bundle") or {})
            ranked = list(state.get("ranked_product_list") or [])

            def _run() -> list[dict[str, Any]]:
                return run_supply_planner(
                    ranked,
                    list(bundle.get("vendors") or []),
                    list(bundle.get("transfers") or []),
                    internal_metrics=list(bundle.get("internal_metrics") or []),
                )

            actionable = await asyncio.to_thread(_run)
            self.repository.add_agent_log(
                user_id=user_id,
                agent_name="Supply_Planner_Agent",
                message=f"Node 5: {len(actionable)} actionable row(s).",
                log_type="info",
            )
            return {"final_actionable_list": actionable}
        finally:
            _discovery_timing_print("Node 5 (Supply Planner)", t0)
    ...
    async def _summary_agent(self, state: DiscoveryState) -> DiscoveryState:
        ...
        try:
            ...
            topic = derive_scan_topic(lanes)
            ranked = list(state.get("ranked_product_list") or [])
            actionable = list(state.get("final_actionable_list") or [])

            def _run() -> str:
                md, _, _ = run_summary_agent(topic=topic, actionable=actionable, ranked_full=ranked)
                return md

            md = await asyncio.to_thread(_run)
            self.repository.add_agent_log(
                user_id=user_id,
                agent_name="Summary_Agent",
                message="Node 6: executive summary (Markdown).",
                log_type="info",
            )
            return {"executive_summary": md}
        finally:
            _discovery_timing_print("Node 6 (Summary)", t0)
    ...
    async def run(
        ...
        *,
        ...
        async with AsyncSqliteSaver.from_conn_string(settings.checkpoints_path) as checkpointer:
            ...
            graph.add_node("trend_analyst_agent", self._trend_analyst_agent)
            graph.add_node("supply_planner_agent", self._supply_planner_agent)
            ...
            graph.add_edge("nlp_compressor", "trend_analyst_agent")
            graph.add_edge("trend_analyst_agent", "supply_planner_agent")
            graph.add_edge("supply_planner_agent", "summary_agent")
